...
<%@ Page language="c#" Codebehind="editComponent.aspx.cs" AutoEventWireup="false" Inherits="BWA.BFP.Web.equip._editComponent" %>
<BFPCNTRLS:HEADER id="Header" RightMenuSelected="0" LeftMenuSelected="0" SubTopMenuSelected="0" TopMenuSelected="1"
...
<form id="Form1" method="post" runat="server">
	<TABLE id="tblComponent">
			...
			<ItemTemplate>
				...
				<TR>
					...
					<TD class="RowElem">
						...
						<asp:RequiredFieldValidator id="rfvValue" runat="server" ErrorMessage='<%# Convert.ToString(DataBinder.Eval(Container.DataItem, "ValueName", "{0}")) + " is required" %>' ControlToValidate="tbValue" Display="Dynamic" Enabled='<%# Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "TB_Visible", "{0}")) %>'>
						...
						<cc1:activedatetime id=adtValue runat="server" Visible='<%# Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "DT_Visible", "{0}")) %>' Date='<%# Convert.ToDateTime(DataBinder.Eval(Container.DataItem, "Value_DateTime", "{0}")) %>' MinYear="1970" MaxYear="2010" Format="MONTH;/;DAY;/;YEAR" AlertEnabled="True" AllowNull="False">
						</cc1:activedatetime>
					</TD>
					<TD class="RowHelp">
		...
		<TR>
			<TD></TD>
			<TD>
				<SPAN onclick="javascript:return confirm('Are you sure that you want to remove this Component from Equipment?')">

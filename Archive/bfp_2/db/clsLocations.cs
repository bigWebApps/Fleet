...
{
	public class clsLocations : clsDBInteractionBase
	{
		...
		public DataTable GetLocationsList()
		{
			...
			scmCmdToExecute.CommandText = "dbo.[sp_SelectLocationsList]";
			...
			catch(Exception ex)
			{
				...
				throw new Exception("clsLocations::GetLocationsList::Error occured.\n" + ex.Message, ex);
			}
			finally
			{
				if(m_bMainConnectionIsCreatedLocal)
		...
		{
			SqlCommand	scmCmdToExecute = new SqlCommand();
			scmCmdToExecute.CommandText = "dbo.[sp_LocationDetail]";
			scmCmdToExecute.CommandType = CommandType.StoredProcedure;
			...
			try
			{
				...
				{
					// Throw error.
					throw new Exception("Stored Procedure 'sp_LocationDetail' reported the ErrorCode: " + m_iErrorCode);
				}
			...
			{
				// some error occured. Bubble it to caller and encapsulate Exception object
				throw new Exception("clsLocations::LocationDetail::Error occured.\n" + ex.Message, ex);
			}
			finally
			{
				if(m_bMainConnectionIsCreatedLocal)

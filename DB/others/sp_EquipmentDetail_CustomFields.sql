...
ALTER   PROCEDURE sp_EquipmentDetail_CustomFields
	(
		@Action char(1),
		@OrgId int,
		@Id int=null,
...
AS
	...
	if @Action='S'
	begin -- select 
		if not exists(select 'true' from Equipments E inner join EquipData ED 
			on E.[DataId] = ED.[Id] and ED.[OrgId] = @OrgId
	...
	begin -- update EquipData
		...
		begin
			...
			and 	Equipments.[OrgId] = @OrgId
			and 	EquipData.[Id] = Equipments.[DataId]

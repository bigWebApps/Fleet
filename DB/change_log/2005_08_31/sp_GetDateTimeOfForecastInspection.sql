...
---------------------------------------------------------------------------
create procedure dbo.sp_GetDateTimeOfForecastInspection
	(
		@OrgId int,
...
as
	...
	begin	
		select 	@dtMaxDate = dateadd(d, datediff(d, @dtTargetDate, @dtTodayDate), dateadd(d, datediff(d, '', @dtTodayDate), '')),
	...
	while @i < datediff(d, @dtMinDate, @dtMaxDate)
	begin
		select @dtTmpDate = dateadd(d, @i, @dtMinDate)
...
--	select @WOCount as WorkOrderCount, @ServiceDays as ServiceDays, @OWOPD as OWOPD, @dtLastDate as LastDate, @dtTargetDate as TargetDate, @dtMaxDate as MaximumDate, @dtMinDate as MinimumDate

...
as
	...
	select 	E.[Id],
		...
		case when isnull(E.TempOperatorId, 0) = 0 then 'green' else 'red' end as Available,
		case when isnull(E.TempOperatorId, 0) = 0 then 'True' else 'False' end as VisibleLink,
		case when isnull(E.EquipOperatorId, 0) = 0 then '' else O.vchLastName + ', ' + O.vchFirstName end as EquipOperatorName,
		case when isnull(E.TempOperatorId, 0) = 0 then '' else U.vchLastName + ', ' + U.vchFirstName end as TempOperatorName
	from Equipments E
	...
	left outer join Logins O
	on O.[Id] = E.EquipOperatorId
	left outer join Logins U
	on U.[Id] = E.TempOperatorId

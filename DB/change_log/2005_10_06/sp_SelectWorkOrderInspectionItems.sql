...
as
	...
	select 	WII.[Id],
		...
		case when GR.m = II.tintOrder then '		<tr valign="middle" class="ag03">
		...
		' else '' end as Header,
		case when GR.m = II.tintOrder then IC.vchName else '' end as CatName,
		case when GR.m = II.tintOrder then '#9E755F' else '#CCCCCC' end as BorderColor,
		case when GR.m = II.tintOrder then 'display:' else 'display:none' end as VisibleSelectButtonOkay,
		case when (GR.m + 1) = II.tintOrder then 'display:' else 'display:none' end as VisibleSelectButtonFair,
		case when GR.m = II.tintOrder then 'javascript:CheckOkayCat("okay_div' + convert(varchar, IC.[Id]) + '");' else '' end as OkayScript,
		case when (GR.m + 1) = II.tintOrder then 'javascript:CheckFairCat("fair_div' + convert(varchar, IC.[Id]) + '");' else '' end as FairScript,
	...
	left join (
		select II.InspectCatId, min(II.tintOrder) as m
		from WorkOrderInspectItems WII
		inner join InspectionItems II
		on WII.WorkOrderInspectId = @InspectId
		and WII.OrgId = @OrgId
	...
	order by IC.tintOrder, II.tintOrder asc

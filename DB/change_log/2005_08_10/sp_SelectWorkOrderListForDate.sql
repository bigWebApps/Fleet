...
AS
	...
	select 	WO.[Id] as [Id],
		...
		case when isnull(LO.vchLastName, '') = '' then '' else LO.vchLastName end as OperatorLastName,
	...
	union all
	select 	WO.[Id] as [Id],
		dateadd(d, datediff(d, '', WO.dtClosed), '') as [Date],
		datepart(dw, WO.dtClosed) as [WeekDay],
		isnull(E.vchEquipId, '') as vchEquipId,
		convert(varchar, year(WO.dtCreated)) + '-' + 
		convert(varchar, month(WO.dtCreated)) + '-' + 
		convert(varchar, day(WO.dtCreated)) + '-' + 
	...
	and WO.OrgId = @OrgId
	and WO.btIsActive = 1
	and datediff(minute, @dtMinDate, dateadd(d, 1, dateadd(d, datediff(d, '', WO.dtClosed), ''))) > 0 
	and datediff(minute, dateadd(d, datediff(d, '', WO.dtClosed), ''), @dtMaxDate) > 0 
	left outer join WorkOrderOperatorStatus WOOS
	on WO.OperatorStatusId = WOOS.[Id]
	left outer join Equipments E
	on E.[Id] = WO.EquipId
